...
- **To play face-up:** Pay the card’s color + generic costs from your available Primal Charges; place the card on the battlefield (Entity / Node) or resolve it (Spell) and place it in the discard zone.
- **To play as a resource:** Place the card upside-down into your Resource Zone. It will generate 1 Primal Charge of its Energy each turn while it remains there. You cannot later use that same physical card’s face-up effect while it remains as a resource unless another effect returns it to hand or otherwise changes its state. Players can play only one resource per turn, unless otherwise specified.
---
## Abilities
...
3. **Assign damage:** For each attacking Entity that has multiple defenders assigned, the attacking player assigns total damage across blockers.
4. **Damage assignment & resolution:**
    - Each attacking Entity assigns its Impact damage to the blocking Entity as assigned by the attacker.
    - All blocking Entities assign their Impact damage simultaneously to the attacking Entity they block.
    - If an attacking Entity is **unblocked**, it deals its Impact as damage to the defending player
...
- If an Entity’s accumulated damage during a turn equals or exceeds its Durability, that Entity is destroyed immediately and sent to the discard zone.
